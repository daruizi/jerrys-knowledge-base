...
- [一、 AI 结对编程核心交互词 (日常最高频)](#一-ai-结对编程核心交互词-日常最高频)
- [二、 API 与模型参数 (开发者必懂)](#二-api-与模型参数-开发者必懂)
- [三、 Agent 工程与高级开发 (核心干货)](#三-agent-工程与高级开发-核心干货)
- [四、 AI 协同代码黑话 (Prompt 高效动词表)](#四-ai-协同代码黑话-prompt-高效动词表)
- [五、 传统算法底层概念 (混圈子仅作了解)](#五-传统算法底层概念-混圈子仅作了解)
...
### 2. Zero-shot & Few-shot (零样本 / 少样本提示)
**释义**：**Zero-shot** 指你直接抛出需求让 AI 去“盲猜盲写”；**Few-shot** 指你在提问时，顺带给它贴了 2-3 个成功的标准示例让它“照猫画虎”。
**实战语境**：“这模型太差劲了，Zero-shot 根本听不懂我要什么格式的数据。赶紧在指令里补 3 个成功的 Few-shot 样例对照着学。”

### 3. System Prompt / Instructions (系统提示词 / 系统指令)
...
### 6. Session / Thread (会话 / 线程)
**释义**：你和 AI 的一次完整对话流程。不同 Session 之间的记忆是隔离的。
**实战语境**：“昨天那个 Session 的上下文太长了，AI 开始胡言乱语 (幻觉) 了，我们切一个新 Session 重新挑起话题吧。”

### 7. XML Tags (结构化标签)
**释义**：顶级模型（如 Claude）最擅长的指令编排格式。通过类似 HTML 的尖括号 `<instructions>`、`<old_code>` 将你的提示词分门别类，防止 AI 读串行或被外部输入（如用户恶意的 Prompt 注入）带偏。
**实战语境**：“这几千行代码直接贴给模型它肯定看花了，把旧代码包在 `<old_code>` 标签里，新需求放进 `<instructions>` 里，这叫结构化 Prompt。”

### 8. Codebase Indexing (代码库索引)
**释义**：在 Cursor 或 GitHub Copilot Workspace 中，为了让 AI 懂你整个项目的架构，IDE 在后台扫描你所有的代码文件并把它们转为向量的动作。
**实战语境**：“为什么我问 Cursor 它老说找不到这几个文件？哦，那个分支我昨天刚拉下来，它记忆库过期了，手动触发一下全盘的 Codebase Indexing (索引重组) 就好了。”
...
**释义**：一个模型一次性最多能记住和处理多少个输入 Token。超出这个窗口的内容会被模型直接“遗忘”。
**实战语境**：“Claude 3.5 Sonnet 支持 200K 的 Context Window，这意味着你可以直接把一本半的书或者一个中型项目的源码一次性塞给它分析。”

### 3. Max Tokens (最大输出限制)
**释义**：模型在**一次回答中**最多能吐出多少个 Token。注意区分它与 Context Window 的区别（一个是输入上限，一个是单次回复上限）。
**实战语境**：“这破模型写了一半的代码突然断了！原来是撞到了 4096 的 Max Tokens 输出上限，赶紧发 'continue' 让它继续写。”

### 4. Temperature 与 Top-p (创造力控制参数)
**释义**：**Temperature** 控制 AI 回答的“发散随机性”（范围通常 `0-2`）。**Top-p** 控制候选词的“概率池大小”。两者通常只需调节一个。
**实战语境**：“要求 AI 帮我严格提取清洗后的 JSON 数据，必须把 Temperature 设置为 `0`（绝对确定不乱编）；如果是让它写脑洞大开的故事，调到 `0.8`。”

### 5. Streaming (流式输出)
**释义**：让 AI 像打字机一样，想出一个字就吐出一个字的技术，而不是等了几十秒才一次性给出完整的长篇大论。
**实战语境**：“我们现在的聊天 UI 体验太差了，用户等得以为卡死了，赶紧把接口改成 Streaming 模式首字秒出。”

### 6. Prompt Caching (提示词/上下文缓存)
...
### 7. Rate Limit (并发拦截 / 速率限制)
**释义**：AI 厂商为了防刷接口，限制你每分钟能发几次请求 (RPM)，或每分钟能消耗多少 Token (TPM)。
**实战语境**：“程序崩溃了！原来是我们在循环里高频调用接口，触发了 Rate Limit 的 429 报错。”

### 8. Reasoning Model (推理模型 / 慢思考模型) 与 CoT
**释义**：2025年最新范式（如 OpenAI o1, DeepSeek R1）。它利用 **CoT (Chain of Thought / 思维链)** 技术，在给出最终答案前先生成一段内化的思考过程（比如“Let's think step by step”的进阶版）。
> [!CAUTION]
> **实战大坑**：面对深度推理模型，**极其忌讳**像以前一样给它写充满繁杂细枝末节的“保姆级” Prompt。你只需要告诉它**“最终核心目标 (What)”** 和 **“硬性约束红线”**，至于实现路径 **(How)**，让它自己去“慢思考”！
**实战语境**：“用 DeepSeek R1 解这种高阶算法题，不要去干涉它的解题流，丢一个纯粹的终极需求过去，等它在后台疯狂转圈慢慢推理完就行了。”
...
### 6. HITL (Human-in-the-loop / 人机协同机制)
**释义**：在高度自动化的 Agent 工作流（如删除数据库、给真实客户发邮件、线上发版发布）中，为了防止 AI 犯下无法挽回的错误，强制设置一个“等待人类点击同意/拒绝”的拦截断点。
**实战语境**：“你写的这个自动化运维 Skill 太疯狂了！在跑到 `npm publish` (发代码包) 那一步时，必须加上 HITL 机制，等我在钉钉上点确认后你才能真正推上线。”

### 7. FIM (Fill-in-the-Middle / 中间填充)
**释义**：当你在 Cursor 或 Copilot 里敲代码时，按下 `Tab` 键瞬间补全一行灰字的黑魔法。区别于普通的聊天大模型（只懂往下文接话），FIM 专门能同时理解你光标“前面”的内容和“后面”的内容，用极小的参数量去极其快速地补足中间挖空的部分。
**实战语境**：“不要用动辄百亿参数的聊天模型去做代码补全（Auto-complete），太慢了！这种需要百毫秒级响应的任务，必须换成专门训练过 FIM (补全) 任务的小尺寸代码模型。”
...
| **Test** | 编写测试 | "Generate Jest unit tests for this component with all edge cases covered." (为这个组件生成覆盖所有边缘情况的 Jest 单元测试) |
| **Document**| 添加注释 | "Add JSDoc comments to all public methods in this class." (为这个类中的所有公开方法添加 JSDoc 注释) |
