...
- [一、 服务器登入与环境体检](#一-服务器登入与环境体检)
- [二、 钉钉接血：Stream 模式配置与寻人](#二-钉钉接血stream-模式配置与寻人)
- [三、 配置最强大脑：阿里云 `qwen-max` 接入](#三-配置最强大脑阿里云-qwen-max-接入)
- [四、 高阶实战：让 AI 处理并真实转发消息](#四-高阶实战让-ai-处理并真实转发消息)
- [五、 点火测试与 24 小时守护进程部署](#五-点火测试与-24-小时守护进程部署)
...
> [!TIP]
> 如果提示未找到命令 (command not found)，请执行以下命令一键安装 Node 环境 (基于 Ubuntu/Debian)：
...
```
> [!TIP]
> 如果提示找不到命令，对 Debian/Ubuntu 系统，执行 `sudo apt-get install python3` 安装即可。
...
> [!CAUTION]
> **安全军规：限制密钥文件权限**
> 虽然是自己的轻量服务器，但让存有真实 Token 的文件“裸奔”是大忌。在终端再敲入一行命令，限制只有你自己能读取这个配置文件：
> `chmod 600 ~/.openclaw/openclaw.json`

> [!IMPORTANT]
> **防急躁提示：配置已就绪，但请先别开机！**
> 在上面的系统提示词里，我们告诉大模型去执行 `~/my-tools/forward.py` 这个剧本文件。但现在这个文件**其实还不存在**！
> 如果你激动地立刻跑去测试，大模型会直接报错说找不到系统命令。让我们紧接着进入第四大步，为它亲自制作这个发消息的“手”。
...
> [!CAUTION]
> **【致命防坑警告】**：在机器人后台的“安全设置”中，**只能勾选“加签”这一个选项！绝对不要勾选“自定义关键词”或“IP地址段”**，否则后续发送 100% 会被钉钉拦截报错！
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        # 注意：Linux 系统下 sys.argv 有参数长度上限（通常为 128KB），处理普通长篇总结足够用。
        # 发送数万字超长长文时，未来可进阶优化为通过文件路径或 stdin 管道符读取。
